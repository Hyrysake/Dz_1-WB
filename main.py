...
class ConsoleView(Observer):
    def update(self, data):
        print(f"ConsoleView: {data}")


class CardView(Observer):
    def update(self, data):
        print(f"CardView: {data}")


class CommandInfoView(Observer):
    def update(self, data):
        print(f"CommandInfoView: {data}")
...
class Record:
    ...
    def add_email(self, email):
        if isinstance(email, EmailAddress):
            ...
            self.emails.append(EmailAddress(None))
            print("Invalid email number format")
...
class AddressBook(UserDict):
    ...
    def add_record_1(self, record):
        self.data[record.name.get_value()] = record
        self.notify_observers(f"Record added: {record.name.get_value()}")
...
def main():
    ...
    # Додаємо спостерігачів (вигляди)
    console_view = ConsoleView()
    card_view = CardView()
    command_info_view = CommandInfoView()

    # Додаємо спостерігачів до адресної книги
    address_book.add_observer(console_view)
    address_book.add_observer(card_view)
    address_book.add_observer(command_info_view)
